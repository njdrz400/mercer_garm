...
The `g-arm_simulation` package creates a virtual replica of the G-Arm robot that can be used for:
...
```bash
cd ~/ros2_ws
colcon build --packages-select g-arm_simulation
...
```bash
ros2 launch g-arm_simulation gazebo.launch.py
```
...
```bash
ros2 launch g-arm_simulation gazebo_moveit.launch.py
```
